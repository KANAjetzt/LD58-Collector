...
func start_task(index := 0) -> void:
	...
	job.steps[index].completed.connect(_on_job_completed.bind(index))
	if job.steps[index].has_signal("aborted"):
		job.steps[index].completed.connect(_on_job_aborted.bind(index))
	job.steps[index].start()


func _on_job_completed(local_step_index := -1) ->  void:
	job.steps[local_step_index].completed.disconnect(_on_job_completed)
	local_step_index += 1
...
func _on_job_aborted(local_step_index := -1) -> void:
	stop()
	job.steps[local_step_index].completed.disconnect(_on_job_aborted)
